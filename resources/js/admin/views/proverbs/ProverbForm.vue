...
function pristine() {
    return {
        category_id: props.proverb?.category_id ?? '',
        question: props.proverb?.question ?? '',
        answer: props.proverb?.answer ?? '',
        answer_aliases: props.proverb?.answer_aliases ?? '',
        difficulty: props.proverb?.difficulty ?? 'easy',
        source: props.proverb?.source ?? '',
    };
}
            ...
            <div class="mt-10 w-full max-w-lg overflow-hidden rounded-xl bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                    <h3 class="text-lg font-semibold text-gray-900">
                        {{ proverb ? 'Edit Proverb' : 'New Proverb' }}
                    </h3>
                ...
                <form class="space-y-4 px-6 py-5" @submit.prevent="submit">
                    <div v-if="duplicate" class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
                        ...
                        <p class="text-sm text-amber-800">
                            A proverb with this answer already exists:
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Answer aliases</label>
                        <input
                            v-model="form.answer_aliases"
                            type="text"
                            class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        />
                        <p class="mt-1 text-xs text-gray-400">Alternative accepted answers, separated by " / ".</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="mt-1 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        />
                        <p class="mt-1 text-xs text-gray-400">Attribution, e.g. a book or a website.</p>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button
                            ...
                            :disabled="saving"
                        >
                            {{ saving ? 'Saving...' : proverb ? 'Save changes' : 'Create proverb' }}
                        </button>
                    </div>
                </form>
            </div>
