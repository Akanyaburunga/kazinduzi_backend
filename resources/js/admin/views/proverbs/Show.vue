...
onMounted(async () => {
    try {
        const { data } = await axios.get(`/admin/api/proverbs/${id.value}/stats`);
...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <button class="mb-2 text-sm text-indigo-600 hover:underline" @click="router.push({ name: 'admin.proverbs.index' })">
                    ← Back to proverbs
                </button>
                <h1 class="text-2xl font-bold text-gray-900">Proverb analytics</h1>
                <p v-if="stats?.proverb" class="mt-1 text-sm text-gray-500">{{ stats.proverb.question }}</p>
            </div>
        </div>
