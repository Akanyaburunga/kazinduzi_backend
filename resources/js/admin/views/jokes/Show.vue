...
onMounted(async () => {
    try {
        const { data } = await axios.get(`/admin/api/jokes/${id.value}/stats`);
...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <button class="mb-2 text-sm text-indigo-600 hover:underline" @click="router.push({ name: 'admin.jokes.index' })">
                    ← Back to jokes
                </button>
                <h1 class="text-2xl font-bold text-gray-900">Joke analytics</h1>
                <p v-if="stats?.joke" class="mt-1 text-sm text-gray-500">{{ stats.joke.setup }}</p>
            </div>
        </div>
