    ...
    <div class="space-y-4">
        ...
        >
            ...
            <template #cell-distractors="{ row }">
                <div v-if="row.distractors?.length" class="flex flex-wrap gap-1">
                    <span v-for="(d, i) in row.distractors" :key="i" class="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600">{{ d }}</span>
                </div>
            ...
            <template #actions="{ row }">
                <div class="flex justify-end gap-2">
                    ...
                    <button class="text-sm text-sky-600 hover:underline" @click="router.push({ name: 'admin.jokes.show', params: { id: row.id } })">Analytics</button>
                    <button
                        v-if="row.deleted_at"
